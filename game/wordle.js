...
function handleKeyPress(key) {
  if (key === "←") {
    ...
    updateBox(currentRow, currentCol, key);
    currentGuess += key;
    currentCol++;
  }
}

function updateBox(row, col, letter) {
  const rowDiv = document.querySelector(`.wordle-row[data-row="${row}"]`);
  const box = rowDiv.querySelector(`.wordle-box[data-col="${col}"]`);
  if (box) {
...
function checkGuess() {
  ...
  if (guess === answer) {
    colorRow(currentRow, guess, answer);
  ...
  if (currentRow >= maxAttempts) {
    msg.textContent = `❌ The word was "${answer}"`;
  }
}

function colorRow(row, guess, answer) {
  const rowDiv = document.querySelector(`.wordle-row[data-row="${row}"]`);
  for (let i = 0; i < 5; i++) {
...
function priority(color) {
  if (color === "#4ade80") return 3;
  if (color === "#facc15") return 2;
  return 1;
}

window.addEventListener("DOMContentLoaded", startWordle);
